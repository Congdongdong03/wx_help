...
function useVirtualList(list, options) {
    ...
    const [scrollTop, setScrollTop] = (0, taro_1.useState)(initialScrollTop);
    const containerRef = (0, taro_1.useRef)(null);
    // 计算可见区域能显示的项目数量
    ...
    // 处理滚动事件
    const handleScroll = (0, taro_1.useCallback)(() => {
        if (containerRef.current) {
            setScrollTop(containerRef.current.scrollTop);
        }
    }, []);
    // 滚动到指定索引
    const scrollToIndex = (0, taro_1.useCallback)((index) => {
        if (containerRef.current) {
